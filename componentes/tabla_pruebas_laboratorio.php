 ...
 <?php
    ...
    $sqlPacientes = "SELECT `prueba`.*, `pacientes`.`nombre`, `pacientes`.`codigo`, `consultas`.`id`, `tipo_prueba`.`nombre_prueba`, `tipo_prueba`.`id` FROM `prueba` LEFT JOIN `pacientes` ON `prueba`.`paciente` = `pacientes`.`id` LEFT JOIN `consultas` ON `consultas`.`paciente_id` = `pacientes`.`id` LEFT JOIN `tipo_prueba` ON `prueba`.`id_tipo_prueba` = `tipo_prueba`.`id` where pagado=1";
