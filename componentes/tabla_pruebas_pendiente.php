 ...
 <?php
    ...
    $sqlPacientes = "SELECT `prueba`.*, `pacientes`.`nombre`, `pacientes`.`codigo`, `consultas`.`id`, `tipo_prueba`.`nombre_prueba`, `tipo_prueba`.`id` FROM `prueba` LEFT JOIN `pacientes` ON `prueba`.`paciente` = `pacientes`.`id` LEFT JOIN `consultas` ON `consultas`.`paciente_id` = `pacientes`.`id` LEFT JOIN `tipo_prueba` ON `prueba`.`id_tipo_prueba` = `tipo_prueba`.`id` WHERE estado=0 and  pagado=1";
                             ...
                             <tbody>
                                 <?php while ($row_pacientes = $pacientes->fetch_assoc()) {  ?>
                                     ...
                                     <tr>
                                         ...
                                         <a href="../LABORATORIO/registra_resultado.php?id_prueba=<?= $row_pacientes['id_prueba'];  ?>   & id_tipo_prueba=<?= $row_pacientes['id'];  ?>  " class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
